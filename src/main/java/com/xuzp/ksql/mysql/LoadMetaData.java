package com.xuzp.ksql.mysql;

import com.alibaba.fastjson.JSON;
import com.google.common.collect.Lists;
import com.xuzp.ksql.DBInfo;
import com.xuzp.ksql.DBTable;
...
public class LoadMetaData {
    ...
    private static final Logger log = LoggerFactory.getLogger(LoadMetaData.class);
    ...
    private static DBInfo loadDataBaseInfo(DatabaseMetaData dbmd) throws Exception {
        DBInfo dbInfo = new DBInfo();
        dbInfo.setDatabaseProductName(dbmd.getDatabaseProductName());
        dbInfo.setDatabaseProductVersion(dbmd.getDatabaseProductVersion());
        dbInfo.setCatalogSeparator(dbmd.getCatalogSeparator());
        dbInfo.setDriverVersion(dbmd.getDriverVersion());
        ResultSet rs = dbmd.getCatalogs();//取可在此数据库中使用的类别名,在mysql中说白了就是可用的数据库名称，只有一列
        List<String> catalogs = Lists.newArrayList();
        while (rs.next()) {
            catalogs.add(rs.getString(1));
        }
        dbInfo.setCatalogs(catalogs);
        log.info("Load DBInfo: {}", JSON.toJSONString(dbInfo));
        return dbInfo;
    }

    private static DBTable loadDBTableInfo(DatabaseMetaData dbmd, String catalog, String tableName) throws Exception {
        ...
        int col = 1;
        while (rs.next()) {
            log.info("DB:{} -> Table: {}", rs.getString(3), rs.getString(4));
            dbTable.setTableCat(rs.getString(col++));
            dbTable.setTableScheme(rs.getString(col++));
            dbTable.setTableName(rs.getString(col++));
            dbTable.setTableType(rs.getString(col++));
            dbTable.setRemark(rs.getString(col++));
            dbTable.setTableTypeCat(rs.getString(col++));
            dbTable.setTableTypeScheme(rs.getString(col++));
            dbTable.setTableTypeName(rs.getString(col++));
